...
int sockets::createNonblockingSocket() {
    ...
    if (fd < 0) {
        critical("{}-{}-{} createNonblockingSocket() failed to create socket: {}\n\n", __FILE__, __func__, __LINE__,
                 strerror(errno));
    }
    return fd;
}
...
struct sockaddr_in sockets::getLocalAddr(int sockfd) {
    ...
    if (::getsockname(sockfd, reinterpret_cast<struct sockaddr*>(&localAddr), &len) < 0) {
        critical("{}-{}-{} getLocalAddr() failed to get local address: {}\n\n", __FILE__, __func__, __LINE__,
                 strerror(errno));
    }
    return localAddr;
}
...
struct sockaddr_in sockets::getPeerAddr(int sockfd) {
    ...
    if (::getpeername(sockfd, reinterpret_cast<struct sockaddr*>(&peerAddr), &len) < 0) {
        critical("{}-{}-{} getPeerAddr() failed to get peer address: {}\n\n", __FILE__, __func__, __LINE__,
                 strerror(errno));
    }
    return peerAddr;
}
